const { PrismaClient, Role, PeriodStatus, AssignmentType } = require("@prisma/client");
...
async function main() {
  ...
  console.log(`✅ Created ${evaluators.length + 1} users`);
  ...
  const employees = await Promise.all(
    employeeData.map((emp) =>
      prisma.employee.upsert({
        ...
        update: {},
        create: { ...emp, startDate: new Date("2024-01-01"), status: "active" },
      })
    )
  );
  ...
  if (!scaleExists) {
    await prisma.scoreScale.create({
      data: {
        name: "มาตรวัด 1-5",
        minScore: 1,
        maxScore: 5,
    ...
    console.log("✅ Created score scale (1-5)");
  }

  // 6. Grade Configurations
  ...
  if (categoriesExist === 0) {
    ...
    console.log("✅ Created evaluation categories & questions");
  }
  ...
  const period = await prisma.evaluationPeriod.upsert({
    where: { id: "august-2026-period" },
    update: { status: PeriodStatus.ACTIVE },
    create: {
      id: "august-2026-period",
      name: "สิงหาคม 2026",
